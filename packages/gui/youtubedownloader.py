...
class YoutubeDownloader():
    ...
    def video_download(self):
        if isinstance(self.ytd, YouTube):
            ...
            if stream is not None:
                self.video = stream.download(output_path=self.save_path, filename=self.default_video_name,
                                             skip_existing=False)
                self.audio_download()
                self.combine()
        ...
        if isinstance(self.ytd, YouTube):
            ...
            if stream is not None:
                if self.data_type == "Audio":
                    ...
                    self.audio = stream.download(output_path=self.save_path, filename=self.default_audio_name,
                                                 skip_existing=False)
